...
if __name__ == "__main__":
    ...
    while clipNameIdx <len(clipNames):
        ...
        filename = "/".join(clipName.split('/')[1:])
        if filename in out:
            clipNameIdx += 1
            continue
        ...
        while(cap.isOpened()):
            ...
            elif k == ord('l'): #6  fast forward
                frameNum += speed
                lastFrameNum = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) - 1
                if(frameNum > lastFrameNum):
                    frameNum = lastFrameNum
                cap.set(cv2.CAP_PROP_POS_FRAMES, frameNum)

                print("Jumped to frame %d"%(frameNum))
            elif k == ord('i'): #8 Speed up
                speed -= 5
                if(speed < 0):
                    speed = 1
                print('speed: %d'%(speed))
            elif k == ord('k'): #8  slow down
                speed += 5
                print('speed: %d'%(speed))
