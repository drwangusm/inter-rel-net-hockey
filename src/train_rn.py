...
from misc.custom_callbacks import RedirectModel
...
def set_callbacks(training_model, model, output_path, checkpoint_period, batch_size, use_earlyStopping=True, return_attention=False):
    callbacks_list = []
    ...
    modelCheckpoint = RedirectModel(modelCheckpoint, model)
    callbacks_list.append(modelCheckpoint)

    auxModelCheckpoint = AuxModelCheckpoint(filepath)
    auxModelCheckpoint = RedirectModel(auxModelCheckpoint, model)
    ...
    modelCheckpoint_val_acc = ModelCheckpoint(filepath, verbose=0,
                    ...
                    save_weights_only=True, period=checkpoint_period)
    modelCheckpoint_val_acc = RedirectModel(modelCheckpoint_val_acc, model)
    callbacks_list.append(modelCheckpoint_val_acc)
    
    auxModelCheckpoint_val_acc = AuxModelCheckpoint(filepath)
    auxModelCheckpoint_val_acc = RedirectModel(auxModelCheckpoint_val_acc, model)
...
def train_model(training_model, model, verbose, learning_rate, output_path, checkpoint_period,
    ...
    #todo check here
    if return_attention:
        training_model.compile(loss=['categorical_crossentropy', None], # Don't train attention
    ...
    callbacks_list = set_callbacks(training_model, model, output_path, checkpoint_period, batch_size,
    ...
    if use_data_gen:
        ...
        fit_history = training_model.fit_generator(train_generator,
            epochs=epochs,
        ...
        fit_history = training_model.fit(X_train, Y_train,
...
def train_rn(output_path, dataset_name, model_kwargs, data_kwargs,
    ...
    fit_history = train_model(training_model = training_model, model=model, verbose=verbose, learning_rate=learning_rate,
