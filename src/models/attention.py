...
class IRNAttentionMLP(keras.layers.Layer):
    ...
    def build(self, input_size):
        ...
        self.w1 = self.add_weight(name="Att1", shape=(input_size[0][1], self.projection_size),
                                  initializer="normal")  # name property is useful for avoiding RuntimeError: Unable to create link.
...
class IRNAttentionTrans(keras.layers.Layer):
    ...
    def build(self, input_size):
        ...
        self.w1 = self.add_weight(name="Att1", shape=(input_size[0][1], self.projection_size),
                                  initializer=initializer_l)  # name property is useful for avoiding RuntimeError: Unable to create link.
        self.w2 = self.add_weight(name="Att2", shape=(input_size[0][1], self.projection_size),
                                  initializer=initializer_l)
        # self.w3=self.add_weight(name="Att3", shape=(input_size[0][1], self.projection_size), initializer="normal") #name property is useful for avoiding RuntimeError: Unable to create link.
...
#         # todo add bias
#
#         self.w1 = self.add_weight(name="Att1", shape=((input_size[0][1] * self.num_obj), self.projection_size),
#                                   initializer="normal")  # name property is useful for avoiding RuntimeError: Unable to create link.
